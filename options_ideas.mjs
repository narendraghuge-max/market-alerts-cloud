...
function pickTier(chain, type, nowMs, target, score, targetDTE) {
  ...
  const cf = score >= 10 ? 0.6 : score >= 8 ? 0.5 : score >= 6 ? 0.4 : 0.3;
...
export async function buildOptionsIdeas(ok, nowMs, n = 4) {
  const bullish = ok.filter(r => !r.leveraged && r.direction !== 'short' && (r.action === 'BUY' || r.action === 'ACCUMULATE' || r.score >= 7)).slice(0, n);
  const bearish = ok.filter(r => !r.leveraged && r.direction === 'short' && (r.action === 'SHORT' || r.action === 'SHORT-SCALE' || r.score >= 7)).sort((a, b) => b.score - a.score).slice(0, n);
  const calls = [], puts = [];
...
export function renderOptionsHtml(optIdeas) {
  ...
  const tierRows = (idea, isCall) => TIERS.map((t, i) => {
    ...
    const symCell = i === 0 ? '<b>' + esc(idea.sym) + '</b><div style="font-size:11px;color:var(--muted)">' + idea.score + '/12 ' + esc(idea.grade || '-') + '</div>' : '';
