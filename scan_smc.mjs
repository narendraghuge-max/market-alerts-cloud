...
function buildReport(rows, errs, exitRows = [], optIdeas = { calls: [], puts: [] }) {
  ...
  try {
    ...
    if (existsSync(gf)) {
      const E = JSON.parse(readFileSync(gf, 'utf8')); const x = E.xray, rk = E.risk;
      const emodes = E.modes || {};
      const modeOrder = ['income', 'conservative', 'balanced', 'aggressive'].filter(k => emodes[k]);
      const defMode = (emodes[E.defaultMode] ? E.defaultMode : (emodes.balanced ? 'balanced' : modeOrder[0] || 'balanced'));
      const hints = { income: 'Park cash for yield &mdash; covered-call income ETFs (QQQI/JEPQ), low drawdown.', conservative: 'Preserve capital &mdash; cut leverage, lean on income + cash.', balanced: 'Risk-first growth &mdash; trim leverage, reduce concentration.', aggressive: 'Stretch growth &mdash; leans into leveraged ETFs; higher upside AND deeper drawdowns.' };
      const shortLabel = { aggressive: 'Aggressive' };
      ...
      const mvColor = a => /sell/i.test(a) ? '#dc2626' : /trim/i.test(a) ? '#d97706' : /add|buy/i.test(a) ? '#16a34a' : /watch/i.test(a) ? '#2563eb' : '#6b7280';
      const usd = n => { n = Math.abs(n); return n >= 1000 ? '$' + (n / 1000).toFixed(1) + 'k' : '$' + Math.round(n); };
      const pl = m => { const buying = /add|buy|watch/i.test(m.action); const p = []; if (m.at) p.push((buying ? 'entry ~$' : 'at ~$') + m.at); if (m.stop != null) p.push((buying ? 'stop $' : 'safety $') + m.stop); if (buying && m.tps && m.tps.length) p.push('TP $' + m.tps.join(' / $')); return p.length ? '<div style="font-size:10px;color:var(--muted);line-height:1.35;margin-top:2px">' + p.join(' &middot; ') + '</div>' : ''; };
      // FLAT cards: each mode emits independent .modecard siblings (no wrapper nesting) tagged data-m, toggled by the selector
      const movesInner = M => (M.moves && M.moves.length) ? '<div class="nl" style="color:var(--up)">Do this &mdash; sized to your book</div><table class="mvtbl"><thead><tr><th>Do</th><th>Ticker &amp; price</th><th class="r">Shares</th><th class="r">Amount</th><th class="r">Target</th></tr></thead><tbody>' + M.moves.map(m => '<tr><td><span class="mvb" style="background:' + mvColor(m.action) + '">' + esc(m.action.toUpperCase()) + '</span></td><td><b>' + esc(m.sym) + '</b>' + (m.note ? '<div style="font-size:10px;color:var(--muted);line-height:1.3">' + esc(m.note) + '</div>' : '') + pl(m) + (m.basis ? '<div style="font-size:10px;color:var(--muted);line-height:1.3;font-style:italic">read: ' + esc(m.basis) + '</div>' : '') + '</td><td class="r" style="color:' + mvColor(m.action) + ';font-weight:700">' + (m.deltaShares > 0 ? '+' : '') + m.deltaShares + '</td><td class="r">~' + usd(m.deltaUsd) + '</td><td class="r" style="color:var(--muted)">' + m.targetPct + '%</td></tr>').join('') + '</tbody></table><div class="nv" style="font-size:11px;color:var(--muted);margin-top:6px"><b>Target</b> = what % of your book each name should be AFTER the move (0% = sell all). Shares &amp; $ sized from current prices. WATCH = only if it triggers.</div>' : '<div class="nl" style="color:var(--up)">Do this</div><div class="nv" style="color:var(--muted)">No sized moves for this mode right now &mdash; hold and watch.</div>';
      const mc = (key, inner, border) => { const st = (border ? 'border-left:3px solid ' + border + ';' : '') + (key !== defMode ? 'display:none' : ''); return '<div class="ncard modecard" data-m="' + key + '"' + (st ? ' style="' + st + '"' : '') + '>' + inner + '</div>'; };
      const modeCards = key => {
        const M = emodes[key], g = M.goal, proj = M.proj;
        const c1 = mc(key, '<div class="modehint" style="text-align:left;margin:1px 0 9px">' + (hints[key] || '') + ' &middot; target ~' + M.target + '%/mo</div>' + movesInner(M) + (M.blocked && M.blocked.length ? '<div class="nv" style="font-size:11px;color:#d97706;margin-top:8px">&#128683; Engine blocked ' + M.blocked.length + ' add' + (M.blocked.length > 1 ? 's' : '') + ' (discipline): ' + M.blocked.map(b => '<b>' + esc(b.sym) + '</b> (' + esc(b.reason) + ')').join(', ') + ' &mdash; won\'t average down into weakness.</div>' : ''), 'var(--up)');
        const c2 = (M.plan && M.plan.length) ? mc(key, '<div class="nl">Why these moves</div><ol style="margin:6px 0 0;padding-left:20px;font-size:13px;line-height:1.55;color:var(--muted)">' + M.plan.map(p => '<li style="margin-bottom:5px">' + esc(p) + '</li>').join('') + '</ol>') : '';
        const c3 = proj ? mc(key, '<div class="nl">Projected return &middot; information only</div><div class="nv" style="font-size:14px"><b>Base case:</b> ~+' + proj.baseMonthly + '%/mo &middot; ~+' + proj.baseAnnual + '%/yr</div><div class="nv" style="margin-top:4px"><b>Likely range</b> for a year (~2 in 3): <span style="color:#dc2626">' + (proj.badYr > 0 ? '+' : '') + proj.badYr + '%</span> to <span style="color:#16a34a">+' + proj.goodYr + '%</span></div>' + (M.read ? '<div class="nv" style="margin-top:7px;color:var(--muted)">' + esc(M.read) + '</div>' : '') + '<div class="nv" style="font-size:11px;color:var(--muted);margin-top:6px">' + M.target + '%/mo &asymp; ' + g.annual + '%/yr &middot; book swings ~' + g.moVolPct + '%/mo &middot; needs a monthly Sharpe of ' + g.moSharpe + ' (elite ~0.3+). Modeling estimate, <b>information only &mdash; not a forecast or guarantee.</b></div>', 'var(--accent)') : '';
        return c1 + c2 + c3;
      };
      const selector = modeOrder.length ? '<div class="modesel">' + modeOrder.map(k => '<button class="modebtn' + (k === defMode ? ' on' : '') + '" data-mode="' + k + '" onclick="inSetMode(\'' + k + '\')">' + esc(shortLabel[k] || emodes[k].label) + '</button>').join('') + '</div>' : '';
      // ---- shared institutional cards (deterministic; describe the current book, same across modes) ----
      const I = E.inst || {};
      const riskMap = (I.contrib && I.contrib.length) ? '<div class="ncard"><div class="nl">Risk map &middot; where your risk really is</div>'
        + I.contrib.slice(0, 6).map(c => { const over = c.riskShare > c.w * 1.4; return '<div style="display:flex;align-items:center;gap:8px;margin:4px 0"><div style="width:52px;font-size:11px;font-weight:600">' + esc(c.sym) + (c.lev > 1 ? '<span style="color:#d97706;font-size:9px"> ' + c.lev + 'x</span>' : '') + '</div><div style="flex:1;background:var(--line);border-radius:5px;height:9px;overflow:hidden"><div style="width:' + Math.min(c.riskShare, 100) + '%;height:100%;background:' + (over ? '#dc2626' : '#2563eb') + '"></div></div><div style="width:104px;text-align:right;font-size:10.5px;color:var(--muted)">' + c.w + '% wt &rarr; <b style="color:' + (over ? '#dc2626' : 'var(--fg)') + '">' + c.riskShare + '% risk</b></div></div>'; }).join('')
        + '<div class="nv" style="font-size:11px;color:var(--muted);margin-top:6px">Risk share = how much each name drives your <b>total</b> risk (weight &times; volatility &times; leverage). Red = eats far more risk than its size &mdash; where a shock hits hardest.</div></div>' : '';
      const mcall = I.marginCall || {}, stress = I.stress || [];
      const marginHtml = stress.length ? '<div class="ncard" style="border-left:3px solid ' + (mcall.callMovePct && mcall.callMovePct < 20 ? '#dc2626' : '#d97706') + '"><div class="nl">Margin call &amp; stress test</div>'
        + '<div class="nv" style="font-size:14px">' + (mcall.callMovePct ? 'A ~<b style="color:#dc2626">' + mcall.callMovePct + '% broad tech/semis selloff</b> triggers a margin call.' : 'Set your maintenance margin to see the call distance.') + (mcall.hasData ? ' <span style="color:var(--muted);font-size:12px">&middot; buffer to maintenance ' + usd(mcall.buffer) + '</span>' : '') + '</div>'
        + '<table class="mvtbl" style="margin-top:8px"><thead><tr><th>If book drops</th><th class="r">Your equity after</th><th class="r">Drawdown</th></tr></thead><tbody>' + stress.map(s => '<tr><td>&minus;' + s.drop + '%</td><td class="r">' + usd(s.equityAfter) + (s.call ? ' <span style="color:#dc2626;font-weight:700">CALL</span>' : '') + '</td><td class="r" style="color:#dc2626">&minus;' + Math.abs(s.ddPct) + '%</td></tr>').join('') + '</tbody></table>'
      ...
      engHtml = '<h2 style="font-size:16px;font-weight:600;margin:14px 0 6px">&#9881; Portfolio Engine <span style="font-size:12px;font-weight:400;color:var(--muted)">&middot; pick your risk mode</span></h2>'
        + '<div class="egrid">' + stat('&plusmn;' + rk.moSwingPct + '%', 'typical swing') + stat('&minus;' + rk.badMonthPct + '%', 'rough bad month', '#dc2626') + stat(x.trueLev + '&times;', 'true leverage', x.trueLev > 1.5 ? '#d97706' : null) + stat('&minus;' + rk.allStopsPct + '%', 'if all stops hit') + '</div>'
        + marginHtml + riskMap + mandateHtml
        ...
        + '<script>(function(){var K="in.engineMode";window.inSetMode=function(k){try{localStorage.setItem(K,k)}catch(e){}var ps=document.querySelectorAll(".modecard"),i;for(i=0;i<ps.length;i++){ps[i].style.display=ps[i].getAttribute("data-m")===k?"":"none"}var bs=document.querySelectorAll(".modebtn"),j;for(j=0;j<bs.length;j++){bs[j].className=bs[j].getAttribute("data-mode")===k?"modebtn on":"modebtn"}};try{var s=localStorage.getItem(K),ok=false,b=document.querySelectorAll(".modebtn"),m;for(m=0;m<b.length;m++){if(b[m].getAttribute("data-mode")===s)ok=true}if(ok){window.inSetMode(s)}}catch(e){}})();</script>';
    }
  } catch (e) {}
  let events = [];
  ...
  if (events.length) {
    const sigColor = s => { s = (s || '').toLowerCase(); return /buy|add/.test(s) ? 'var(--up)' : /watch/.test(s) ? 'var(--warn)' : /avoid|trim|sell/.test(s) ? '#dc2626' : 'var(--muted)'; };
    evHtml += events.map(e => '<div class="ncard"><div class="nh">' + esc(e.headline) + '</div>'
      ...
      + (e.rec ? '<div class="idea" style="border-left-color:' + sigColor(e.signal) + '"><div class="nl" style="color:' + sigColor(e.signal) + '">Trade idea' + (e.signal ? ' &middot; ' + esc(e.signal) : '') + '</div><div class="nv">' + esc(e.rec) + '</div></div>' : '')
  ...
  const __out = '<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">'
    ...
    + '<style>'
    + ':root{--bg:#fff;--fg:#1a1a1a;--muted:#6b7280;--line:#e5e7eb;--card:#f6f7f9;--accent:#2563eb;--up:#16a34a;--warn:#d97706;--poc:#7c3aed}'
    + '@media(prefers-color-scheme:dark){:root{--bg:#0f1115;--fg:#e8e8e8;--muted:#9aa0aa;--line:#272b32;--card:#161922;--accent:#3b82f6;--up:#22c55e;--warn:#f59e0b;--poc:#a78bfa}}'
    ...
    + '.masthead{background:linear-gradient(135deg,rgba(37,99,235,.14),rgba(37,99,235,.02));border:1px solid var(--line);border-radius:14px;padding:15px 17px;margin:0 0 16px}.hdr{display:flex;align-items:center;gap:13px;margin:0}.logo{width:42px;height:42px;flex:none;border-radius:11px;background:linear-gradient(160deg,#11223f,#2563eb);display:flex;align-items:center;justify-content:center;font-weight:800;color:#fff;font-size:17px;letter-spacing:.5px;box-shadow:0 2px 10px rgba(37,99,235,.3)}h1{font-size:21px;font-weight:700;margin:0;letter-spacing:-.2px}.tagline{color:var(--fg);opacity:.82;font-size:12.5px;margin:3px 0 0;line-height:1.35}.sub{color:var(--muted);font-size:13px;margin-bottom:16px}'
    + '.controls{display:flex;flex-wrap:wrap;gap:10px;align-items:center;margin-bottom:12px}'
    + 'select{padding:8px 12px;border:1.5px solid var(--accent);border-radius:9px;background:var(--card);color:var(--fg);font-size:13px;font-weight:600;cursor:pointer}select:focus{outline:none;box-shadow:0 0 0 3px rgba(37,99,235,.25)}select:hover{filter:brightness(1.06)}input#f-sym{padding:8px 12px;border:1.5px solid var(--accent);border-radius:9px;background:var(--card);color:var(--fg);font-size:13px;font-weight:600;width:140px;max-width:42vw}input#f-sym::placeholder{color:var(--muted);font-weight:400}input#f-sym:focus{outline:none;box-shadow:0 0 0 3px rgba(37,99,235,.25)}'
    ...
    + '.mvtbl{width:100%;border-collapse:collapse;font-size:13px;margin-top:7px}.mvtbl th{text-align:left;color:var(--muted);font-weight:500;font-size:9.5px;text-transform:uppercase;letter-spacing:.3px;padding:4px 5px;border-bottom:1px solid var(--line)}.mvtbl td{padding:8px 5px;border-bottom:1px solid var(--line);vertical-align:middle}.mvtbl tr:last-child td{border-bottom:none}.mvtbl th.r,.mvtbl td.r{text-align:right;white-space:nowrap}.mvb{display:inline-block;color:#fff;font-size:9.5px;font-weight:700;padding:2px 6px;border-radius:5px;letter-spacing:.3px;white-space:nowrap}'
    + '.modesel{display:flex;flex-wrap:wrap;gap:5px;margin:12px 0 2px;background:var(--card);border:1px solid var(--line);border-radius:11px;padding:4px}.modebtn{flex:1 1 66px;border:none;background:transparent;color:var(--muted);font-size:11.5px;font-weight:600;padding:8px 3px;border-radius:8px;cursor:pointer;font-family:inherit;white-space:nowrap;transition:background .12s,color .12s}.modebtn.on{background:var(--accent);color:#fff}.modehint{font-size:11px;color:var(--muted);margin:5px 2px 2px;text-align:center}'
    ...
    + '@media(max-width:680px){.cards thead{display:none}.cards,.cards tbody{display:block}.cards tr{display:grid;grid-template-columns:1fr 1fr;column-gap:16px;border:1px solid var(--line);border-radius:12px;margin:10px 0;padding:8px 14px;background:var(--card)}.cards td{display:flex;flex-wrap:wrap;justify-content:space-between;align-items:baseline;column-gap:8px;border:none;padding:6px 0;font-weight:600;font-size:13px;color:var(--fg);min-width:0;overflow-wrap:anywhere}.cards td::before{content:attr(data-label);color:var(--muted);font-weight:500;font-size:10px;text-transform:uppercase;letter-spacing:.4px;white-space:nowrap;align-self:center}.cards td:first-child{position:static;box-shadow:none}.cards td>div{width:100%;text-align:right}.cards td[data-label="Basis"],.cards td[data-label="What to do"],.cards td[data-label="Take-profit"]{grid-column:1/-1}.cards td.basis{display:block;text-align:left}.cards td.basis::before{display:none}.cards td[data-label="Holding"] b{font-size:15px}.cards td[data-label="Gain/loss"]{font-size:14px;font-weight:700}.cards td[data-label="Signal"] .pill,.cards td[data-label="Action"] .pill{font-size:12.5px}.cards td[data-label="Take-profit"],.cards td[data-label="Take-profit"] span{color:var(--up)!important;font-weight:700}.cards td[data-label="Safety price"]{color:var(--warn)!important;font-weight:700}.cards td[data-label="Volume 30d"]{color:var(--poc)!important}}'
    + '</style></head><body>'
    ...
    + '<div class="controls">'
    + '<input id="f-sym" list="symlist" placeholder="Type a ticker..." autocomplete="off" autocapitalize="characters" title="type a ticker to find it"><datalist id="symlist"></datalist>'
    ...
    + 'function render(){var mS=+$("f-score").value,so=$("f-sort").value,q2=$("f-rr").checked,ho=$("f-hold").checked,hl=$("f-lev").checked,sy=($("f-sym").value||"").toUpperCase().trim();'
    + 'var rows;if(sy){rows=DATA.filter(function(r){return r.sym.indexOf(sy)===0})}else{rows=DATA.filter(function(r){return r.isNew||r.score>=mS});if(q2)rows=rows.filter(function(r){return r.isNew||r.rr>=2});if(ho)rows=rows.filter(function(r){return r.hold});if(hl)rows=rows.filter(function(r){return !r.lev})}'
    ...
    + '+"<td data-label=\'Take-profit\' class=r style=\\"color:var(--muted)\\">"+fmt(r.tp1)+(r.read&&r.read.targets?" <span style=\\"font-size:10px\\" title=\\"odds of reaching this target BEFORE the stop = 1/(1+R); a no-edge baseline, your setup edge tilts it higher\\">("+r.read.targets[0].p+"%)</span>":"")+" \\u00b7 <span style=\\"color:var(--fg)\\">"+fmt(r.tp2)+"</span>"+(r.read&&r.read.targets?" <span style=\\"font-size:10px;color:var(--muted)\\">("+r.read.targets[1].p+"%)</span>":"")+" \\u00b7 "+fmt(r.tp3)+(r.read&&r.read.targets?" <span style=\\"font-size:10px;color:var(--muted)\\">("+r.read.targets[2].p+"%)</span>":"")+"</td>"'
  ...
  return __out.replace(/<table\b/g, '<div class="tbl"><table').replace(/<\/table>/g, '</table></div>');
}
