...
function buildReport(rows, errs, exitRows = [], optIdeas = { calls: [], puts: [] }) {
  ...
  try {
    ...
    if (existsSync(gf)) {
      ...
      const modeOrder = ['conservative', 'balanced', 'aggressive'].filter(k => emodes[k]);
      const defMode = (emodes[E.defaultMode] ? E.defaultMode : (modeOrder[1] || modeOrder[0] || 'balanced'));
      const hints = { conservative: 'Preserve capital &mdash; cut leverage, lean on income + cash.', balanced: 'Risk-first growth &mdash; trim leverage, reduce concentration.', aggressive: 'Stretch growth &mdash; leans into leveraged ETFs; higher upside AND deeper drawdowns.' };
      ...
      const panelOf = key => { const M = emodes[key], g = M.goal, proj = M.proj; return '<div class="modepanel" data-mode="' + key + '"' + (key === defMode ? '' : ' style="display:none"') + '>'
        ...
        + '</div>'; };
      const selector = modeOrder.length ? '<div class="modesel">' + modeOrder.map(k => '<button class="modebtn' + (k === defMode ? ' on' : '') + '" data-mode="' + k + '" onclick="inSetMode(\'' + k + '\')">' + esc(emodes[k].label) + '</button>').join('') + '</div>' : '';
  ...
  const __out = '<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">'
    ...
    + '.modesel{display:flex;gap:5px;margin:12px 0 2px;background:var(--card);border:1px solid var(--line);border-radius:11px;padding:4px}.modebtn{flex:1;border:none;background:transparent;color:var(--muted);font-size:12.5px;font-weight:600;padding:9px 4px;border-radius:8px;cursor:pointer;font-family:inherit;transition:background .12s,color .12s}.modebtn.on{background:var(--accent);color:#fff}.modehint{font-size:11px;color:var(--muted);margin:5px 2px 2px;text-align:center}'
