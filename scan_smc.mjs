...
function buildReport(rows, errs, exitRows = [], optIdeas = { calls: [], puts: [] }) {
  ...
  try {
    ...
    if (existsSync(gf)) {
      ...
      const movesTblOf = M => (M.moves && M.moves.length) ? '<div class="ncard" style="border-left:3px solid var(--up)"><div class="nl" style="color:var(--up)">Do this &mdash; sized to your book</div><table class="mvtbl"><thead><tr><th>Do</th><th>Ticker &amp; price</th><th class="r">Shares</th><th class="r">Amount</th><th class="r">Target</th></tr></thead><tbody>' + M.moves.map(m => '<tr><td><span class="mvb" style="background:' + mvColor(m.action) + '">' + esc(m.action.toUpperCase()) + '</span></td><td><b>' + esc(m.sym) + '</b>' + (m.note ? '<div style="font-size:10px;color:var(--muted);line-height:1.3">' + esc(m.note) + '</div>' : '') + pl(m) + '</td><td class="r" style="color:' + mvColor(m.action) + ';font-weight:700">' + (m.deltaShares > 0 ? '+' : '') + m.deltaShares + '</td><td class="r">~' + usd(m.deltaUsd) + '</td><td class="r" style="color:var(--muted)">' + m.targetPct + '%</td></tr>').join('') + '</tbody></table><div class="nv" style="font-size:11px;color:var(--muted);margin-top:6px"><b>Target</b> = what % of your book each name should be AFTER the move (0% = sell all). Shares &amp; $ are sized from current prices to reach it. WATCH = only if it triggers.</div></div>' : '<div class="ncard"><div class="nl" style="color:var(--up)">Do this</div><div class="nv" style="color:var(--muted)">No sized moves for this mode right now &mdash; hold and watch.</div></div>';
      const panelOf = key => { const M = emodes[key], g = M.goal, proj = M.proj; return '<div class="modepanel" data-mode="' + key + '"' + (key === defMode ? '' : ' style="display:none"') + '>'
        + '<div class="modehint">' + (hints[key] || '') + ' &middot; target ~' + M.target + '%/mo</div>'
        + movesTblOf(M)
        + (M.plan && M.plan.length ? '<div class="ncard"><div class="nl">Why these moves</div><ol style="margin:6px 0 0;padding-left:20px;font-size:13px;line-height:1.55;color:var(--muted)">' + M.plan.map(p => '<li style="margin-bottom:5px">' + esc(p) + '</li>').join('') + '</ol></div>' : '')
        + (proj ? '<div class="ncard"><div class="nl">Projected return &middot; information only</div><div class="nv" style="font-size:14px"><b>Base case:</b> ~+' + proj.baseMonthly + '%/mo &middot; ~+' + proj.baseAnnual + '%/yr</div><div class="nv" style="margin-top:4px"><b>Likely range</b> for a year (~2 in 3): <span style="color:#dc2626">' + (proj.badYr > 0 ? '+' : '') + proj.badYr + '%</span> to <span style="color:#16a34a">+' + proj.goodYr + '%</span></div><div class="nv" style="font-size:11px;color:var(--muted);margin-top:6px">A modeling estimate from a ~' + proj.baseAnnual + '%/yr assumption and this mode&#39;s expected swings (~' + proj.annualVol + '%/yr). <b>For information only &mdash; not a forecast or guarantee;</b> real returns will differ and can be negative.</div></div>' : '')
        + (M.read ? '<div class="ncard" style="border-left:3px solid var(--accent)"><div class="nl" style="color:var(--accent)">Goal check &middot; is ' + M.target + '%/mo realistic?</div><div class="nv">' + esc(M.read) + '</div><div class="nv" style="margin-top:5px;color:var(--muted);font-size:12px">' + M.target + '%/mo &asymp; ' + g.annual + '%/yr &middot; book swings ~' + g.moVolPct + '%/mo &middot; needs a monthly Sharpe of ' + g.moSharpe + ' (elite is ~0.3+)</div></div>' : '')
        + '</div>'; };
      ...
      engHtml = '<h2 style="font-size:16px;font-weight:600;margin:14px 0 6px">&#9881; Portfolio Engine <span style="font-size:12px;font-weight:400;color:var(--muted)">&middot; pick your risk mode</span></h2>'
        ...
        + '<script>(function(){var K="in.engineMode";window.inSetMode=function(k){try{localStorage.setItem(K,k)}catch(e){}var ps=document.querySelectorAll(".modepanel"),i;for(i=0;i<ps.length;i++){ps[i].style.display=ps[i].getAttribute("data-mode")===k?"":"none"}var bs=document.querySelectorAll(".modebtn"),j;for(j=0;j<bs.length;j++){bs[j].className=bs[j].getAttribute("data-mode")===k?"modebtn on":"modebtn"}};try{var s=localStorage.getItem(K),ok=false,b=document.querySelectorAll(".modebtn"),m;for(m=0;m<b.length;m++){if(b[m].getAttribute("data-mode")===s)ok=true}if(ok){window.inSetMode(s)}}catch(e){}})();</script>';
    }
  } catch (e) {}
  let events = [];
