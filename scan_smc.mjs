...
function buildReport(rows, errs, exitRows = [], optIdeas = { calls: [], puts: [] }) {
  ...
  try {
    ...
    if (existsSync(gf)) {
      ...
      const movesTbl = (E.moves && E.moves.length) ? '<div class="ncard" style="border-left:3px solid var(--up)"><div class="nl" style="color:var(--up)">Do this &mdash; sized to your book</div><table class="mvtbl"><thead><tr><th>Do</th><th>Ticker &amp; price</th><th class="r">Shares</th><th class="r">Amount</th><th class="r">Target</th></tr></thead><tbody>' + E.moves.map(m => '<tr><td><span class="mvb" style="background:' + mvColor(m.action) + '">' + esc(m.action.toUpperCase()) + '</span></td><td><b>' + esc(m.sym) + '</b>' + (m.note ? '<div style="font-size:10px;color:var(--muted);line-height:1.3">' + esc(m.note) + '</div>' : '') + pl(m) + '</td><td class="r" style="color:' + mvColor(m.action) + ';font-weight:700">' + (m.deltaShares > 0 ? '+' : '') + m.deltaShares + '</td><td class="r">~' + usd(m.deltaUsd) + '</td><td class="r" style="color:var(--muted)">' + m.targetPct + '%</td></tr>').join('') + '</tbody></table><div class="nv" style="font-size:11px;color:var(--muted);margin-top:6px"><b>Target</b> = what % of your book each name should be AFTER the move (0% = sell all). Shares &amp; $ are sized from current prices to reach it. WATCH = only if it triggers.</div></div>' : '';
      engHtml = '<h2 style="font-size:16px;font-weight:600;margin:14px 0 6px">&#9881; Portfolio Engine <span style="font-size:12px;font-weight:400;color:var(--muted)">&middot; aiming for a realistic ' + E.target + '%/mo</span></h2>'
        ...
        + (E.plan && E.plan.length ? '<div class="ncard"><div class="nl">Why these moves</div><ol style="margin:6px 0 0;padding-left:20px;font-size:13px;line-height:1.55;color:var(--muted)">' + E.plan.map(p => '<li style="margin-bottom:5px">' + esc(p) + '</li>').join('') + '</ol></div>' : '')
        + (E.proj ? '<div class="ncard"><div class="nl">Projected return &middot; information only</div><div class="nv" style="font-size:14px"><b>Base case:</b> ~+' + E.proj.baseMonthly + '%/mo &middot; ~+' + E.proj.baseAnnual + '%/yr</div><div class="nv" style="margin-top:4px"><b>Likely range</b> for a year (~2 in 3): <span style="color:#dc2626">' + (E.proj.badYr > 0 ? '+' : '') + E.proj.badYr + '%</span> to <span style="color:#16a34a">+' + E.proj.goodYr + '%</span></div><div class="nv" style="font-size:11px;color:var(--muted);margin-top:6px">A modeling estimate from a ~' + E.proj.baseAnnual + '%/yr long-run market assumption and your book&#39;s own swings (~' + E.proj.annualVol + '%/yr). <b>For information only &mdash; not a forecast or guarantee;</b> real returns will differ and can be negative.</div></div>' : '')
